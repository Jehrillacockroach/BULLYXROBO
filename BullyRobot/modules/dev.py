...
from BullyRobot.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        update.effective_message.reply_text(f"Current state: {BullyRobot.ALLOW_CHATS}")
        return
    if args[0].lower() in ["off", "no"]:
        BullyRobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        BullyRobot.ALLOW_CHATS = False
    else:
