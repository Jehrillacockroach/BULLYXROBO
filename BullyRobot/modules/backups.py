...
import BullyRobot.modules.sql.notes_sql as sql
from BullyRobot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from BullyRobot.__main__ import DATA_IMPORT
from BullyRobot.modules.helper_funcs.chat_status import user_admin
from BullyRobot.modules.helper_funcs.alternate import typing_action

# from BullyRobot.modules.rules import get_rules
import BullyRobot.modules.sql.rules_sql as rulessql

# from BullyRobot.modules.sql import warns_sql as warnssql
import BullyRobot.modules.sql.blacklist_sql as blacklistsql
from BullyRobot.modules.sql import disable_sql as disabledsql

# from BullyRobot.modules.sql import cust_filters_sql as filtersql
# import BullyRobot.modules.sql.welcome_sql as welcsql
import BullyRobot.modules.sql.locks_sql as locksql
from BullyRobot.modules.connection import connected
...
def export_data(update, context):
    ...
    with open("BullyRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("BullyRobot{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `BullyRobot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("BullyRobot{}.backup".format(chat_id))  # Cleaning file
